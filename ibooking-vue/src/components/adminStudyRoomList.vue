<template>
  <div class="studyRoom-List">
    ...
    <div class="container">
      <p class="info3">您所在的位置：首页 - 自习室查询</p>
      <el-row>
        <div class="search">
          ...
          <el-table :data="studyRoomTable">
            ...
            <el-table-column label="操作">
              <template slot-scope="scope">
                <el-button size="mini" type="primary" @click="changeStatus(scope.$index)">修改状态</el-button>
              </template>
            </el-table-column>
            <el-table-column>
              <template slot-scope="scope">
                <el-button size="mini" @click="studyRoomDetail(scope.$index)">查看详情</el-button>
              </template>
            </el-table-column>
          </el-table>
        </div>
  ...
  export default {
    ...
    data () {
      ...
      getFloor (code) {
        ...
        axios.post('/api/admin/changeStudyRoomStatus', {
          id: this.studyRoomTable[index].id,
          status: this.studyRoomTable[index].status === "不可用" ? 1 : 0
        }).then(resp => {
          if (resp.status === 200) {
            this.$message({
              message: '操作成功',
